...
def test_cluster_hook(test_cluster: TestCluster) -> None:  # noqa: D103
    typ = test_cluster.type_system.convert_type_hint(pd.DataFrame)
    ...
    def generate_variable(  # noqa: D102
        ...
        *,
        ...
        columns_number = randomness.next_int(
            pandas_dataframe_min_columns_number, pandas_dataframe_max_columns_number + 1
        )
...
class PandasDataframeStatement(VariableCreatingStatement):
    ...
    def __init__(
        ...
        pandas_dataframe_instance = (
            test_case.test_cluster.type_system.convert_type_hint(pd.DataFrame)
        )
